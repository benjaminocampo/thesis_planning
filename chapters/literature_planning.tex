...
anticipando el resultado de nuestras decisiones, aun si no estamos completamente
conscientes de ello, o explícitamente planeando que hacer antes de realizar una
acción. No obstante, una tarea que abarque objetivos nuevos y complejos requiere
deliberación al consistir de acciones que uno no está acostumbrado, tareas de
alto riesgo, o cooperación con alguien más.

\emph{Automated Planning} es el área de la inteligencia artificial que estudia
...
que ocurren en las acciones y en la meta. En STRIPS, las fórmulas están en primera forma normal conjuntiva \emph{1-CNF}, es decir, son de la forma $\bigwedge_i l_i$ con $l_i$
...
estados, solo se mantienen aquellos que son verdaderos en el estado, es decir, los
...
Algo importante a recalcar es el orden en que se ejecuta una acción. Para un estado aplicable de $A$ optaremos por primero eliminar los símbolos que sean verdaderos a partir de su lista $del$ para luego agregar los de la lista $add$ con el fin de garantizar de que el estado resultante contenga todos los símbolos de esta última lista.
...
PSPACE-completo. Es por eso que las implementaciones que lo resuelven son a partir de algoritmos de búsqueda de orden exponencial. Esta búsqueda es guiada a partir de una \emph{función heurística} informada por \emph{planes relajados}.

Los planes relajados son secuencias de acciones cuyos efectos negativos son eliminados, son aplicables en el estado inicial, y su estado resultante contiene a la meta. Estos proviene de la \emph{relajación por deletes}, una simplificación de la tarea de planning que consiste en
eliminar los efectos negativos de las acciones, es decir, su lista $del$. Esta
...
vuelve verdadero en un estado $s$, entonces todos los estados que se puedan formar a partir de sucesivas aplicaciones de acciones también harán verdadero $p$. Por ejemplo, en el
...
tarea STRIPS resultante sea satisfacible relajadamente a partir de los elementos procesados hasta el momento, esto es, si $G \subseteq F$. Un detalle a notar
